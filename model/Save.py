...
if uploaded_file is not None:
    ...
    # Save the image with boxes around words
    output_image_path = os.path.join(output_folder, 'image_with_word_boxes.jpg')
    ...
    # Save each segmented word
    for i, word in enumerate(word_boxes):
        x1, y1, x2, y2 = word
        word_img = processed_img[y1:y2, x1:x2]
        
        # Save individual word images
        word_output_path = os.path.join(output_folder, f'word_{i + 1}.jpg')
        cv2.imwrite(word_output_path, cv2.cvtColor(word_img, cv2.COLOR_RGB2BGR))

        st.image(word_img, caption=f"Word {i + 1}")
    
    st.write(f"Images saved in '{output_folder}' folder.")
